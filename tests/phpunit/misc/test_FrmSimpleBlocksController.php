...
class test_FrmSimpleBlocksController extends FrmUnitTest {
	...
	public function test_maybe_remove_fade_on_load_for_block_preview() {
		...
		if ( is_callable( 'wp_is_json_request' ) ) {
			$this->assertStringNotContainsString( 'frm_logic_form', (string) $this->maybe_remove_fade_on_load_for_block_preview( $form ) );
		}

		unset( $_SERVER['HTTP_ACCEPT'], $_SERVER['CONTENT_TYPE'] );
		$this->assertStringContainsString( 'frm_logic_form', (string) $this->maybe_remove_fade_on_load_for_block_preview( $form ) );
	}
