<?php
/**
 * Formidable_Sniffs_CodeAnalysis_RedundantEmptyOnStaticPropertySniff
 *
 * Detects redundant empty() calls on static properties like self::$plugin,
 * static::$plugin, or ClassName::$plugin where the property is defined
 * in the class. Since defined static properties are always set, empty()
 * can be simplified to a falsy check.
 *
...
/**
 * Detects redundant empty() calls on defined static properties.
 *
 * Bad:
 * if ( empty( self::$plugin ) ) { ... }
 * if ( ! empty( self::$plugin ) ) { ... }
 * if ( empty( static::$plugin ) ) { ... }
 * if ( empty( MyClass::$plugin ) ) { ... }
 *
 * Good:
 * if ( ! self::$plugin ) { ... }
 * if ( self::$plugin ) { ... }
 ...
 * if ( ! MyClass::$plugin ) { ... }
 */
class RedundantEmptyOnStaticPropertySniff implements Sniff {
	...
	public function process( File $phpcsFile, $stackPtr ) {
		...
		// Check if the argument is a static property access (self::$prop, static::$prop, ClassName::$prop).
		$staticAccess = $this->getStaticPropertyAccess( $phpcsFile, $openParen, $closeParen );

		if ( false === $staticAccess ) {
			return;
		}
		...
		// Check if the property is defined as a static property in the class.
		if ( ! $this->isDefinedStaticProperty( $phpcsFile, $classToken, $staticAccess['property'] ) ) {
			return;
		}
		...
		$propertyAccess = $staticAccess['accessor'] . '::$' . $staticAccess['property'];

		// Determine the suggested replacement.
		if ( $isNegated ) {
			$suggestion = $propertyAccess;
			$message    = 'Redundant empty() on defined static property %s. Use "%s" instead of "! empty( %s )"';
		} else {
			$suggestion = '! ' . $propertyAccess;
			$message    = 'Redundant empty() on defined static property %s. Use "%s" instead of "empty( %s )"';
		}
		...
		// Replace empty( self::$prop ) with self::$prop or ! self::$prop.
	...
	/**
	 * Check if the content inside empty() is a static property access.
	 *
	 * Returns an array with 'accessor' (self, static, or ClassName) and 'property' name
	 * if it matches, false otherwise. Skips array access like self::$prop['key'].
	 *
	 * @param File $phpcsFile  The file being scanned.
	...
	private function getStaticPropertyAccess( File $phpcsFile, $openParen, $closeParen ) {
		...
		// Find the accessor (self, static, or a class name).
		$accessorToken = $phpcsFile->findNext( T_WHITESPACE, $openParen + 1, $closeParen, true );

		if ( false === $accessorToken ) {
			return false;
		}

		$accessorCode = $tokens[ $accessorToken ]['code'];

		if ( $accessorCode !== T_SELF && $accessorCode !== T_STATIC && $accessorCode !== T_STRING ) {
			return false;
		}

		$accessor = $tokens[ $accessorToken ]['content'];

		// Find the :: operator.
		$doubleColonToken = $phpcsFile->findNext( T_WHITESPACE, $accessorToken + 1, $closeParen, true );

		if ( false === $doubleColonToken || $tokens[ $doubleColonToken ]['code'] !== T_DOUBLE_COLON ) {
			return false;
		}

		// Find the $property variable.
		$propToken = $phpcsFile->findNext( T_WHITESPACE, $doubleColonToken + 1, $closeParen, true );

		if ( false === $propToken || $tokens[ $propToken ]['code'] !== T_VARIABLE ) {
			return false;
		}

		$propertyName = ltrim( $tokens[ $propToken ]['content'], '$' );
	...
	/**
	 * Check if a property is defined as a static property in the class.
	 *
	 * Scans the class body for static property declarations.
	 *
	 * @param File   $phpcsFile    The file being scanned.
	 * @param int    $classToken   The position of the class token.
	 * @param string $propertyName The property name to look for (without $).
	 *
	 * @return bool True if the property is defined as static in the class.
	 */
	private function isDefinedStaticProperty( File $phpcsFile, $classToken, $propertyName ) {
		$tokens      = $phpcsFile->getTokens();
		...
		for ( $i = $classOpener + 1; $i < $classCloser; $i++ ) {
			...
			// Look for property declarations with static keyword.
			...
			while ( $j < $classCloser ) {
				...
				if ( $code === T_STATIC ) {
					...
					// Found a variable - check if this is a static property declaration.
					if ( $hasStatic ) {
						$varName = ltrim( $tokens[ $j ]['content'], '$' );
					...
					// Move past this declaration.
					break;
				} elseif ( ! in_array( $code, array( T_PUBLIC, T_PROTECTED, T_PRIVATE, T_VAR, T_WHITESPACE, T_READONLY, T_STRING, T_NULLABLE, T_TYPE_UNION, T_TYPE_INTERSECTION, T_NULL, T_SELF, T_PARENT, T_ARRAY, T_CALLABLE, T_NS_SEPARATOR ), true ) ) {
					// Not a property declaration modifier - stop.
					break;
				}
