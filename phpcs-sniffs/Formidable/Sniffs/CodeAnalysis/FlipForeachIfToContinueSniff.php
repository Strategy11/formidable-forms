<?php
/**
 * Formidable_Sniffs_CodeAnalysis_FlipForeachIfToContinueSniff
 *
 * Detects foreach loops where the entire body is inside a single if statement
 * and suggests flipping to a continue pattern.
 *
...
/**
 * Detects foreach loops that should use continue pattern.
 *
 * Bad:
 ...
 * }
 */
class FlipForeachIfToContinueSniff implements Sniff {
	...
	public function process( File $phpcsFile, $stackPtr ) {
		...
		// Get the foreach's scope opener and closer.
		...
		// If the next token after the if isn't the foreach's closing brace, there's more code.
		...
		$fix = $phpcsFile->addFixableError(
			'Consider using continue pattern. Flip the condition "if (%s)" to "if (! %s) { continue; }" and move the body outside.',
			$ifToken,
			'Found',
	...
	/**
	 * Apply the fix to flip the if statement to continue pattern.
	 *
	 * @param File $phpcsFile      The file being scanned.
	 * @param int  $ifToken        The position of the if token.
	 * @param int  $foreachOpener  The foreach's scope opener.
	 * @param int  $foreachCloser  The foreach's scope closer.
	 *
	 * @return void
	 */
	private function applyFix( File $phpcsFile, $ifToken, $foreachOpener, $foreachCloser ) {
		$tokens = $phpcsFile->getTokens();
		...
		// Build the new code: continue guard + body.
		$newCode  = 'if ( ' . $negatedCondition . ' ) {' . $phpcsFile->eolChar;
		$newCode .= $indent . "\tcontinue;" . $phpcsFile->eolChar;
