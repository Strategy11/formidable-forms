<?php
/**
 * Formidable_Sniffs_CodeAnalysis_SimplifyIssetTruthyCheckSniff
 *
 ...
 * if ( ! empty( $arr['key'] ) ) { ... }
 */
class SimplifyIssetTruthyCheckSniff implements Sniff {
	...
	public function process( File $phpcsFile, $stackPtr ) {
		...
		$secondVarStart = $phpcsFile->findNext( T_WHITESPACE, $andOperator + 1, null, true );
		...
		// Check that there's no comparison operator after the second variable.
		// We only want pure truthy checks like `isset($x) && $x`, not `isset($x) && $x == 'value'`.
		$afterSecondVar = $phpcsFile->findNext( T_WHITESPACE, $secondContent['end'] + 1, null, true );

		if ( false !== $afterSecondVar ) {
	...
	private function getExpressionContent( File $phpcsFile, $startPtr ) {
		...
		// Valid tokens in an expression like $arr['key'] or $obj->prop.
		$validTokens = array(
			T_VARIABLE,
			T_STRING,
