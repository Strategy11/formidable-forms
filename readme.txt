...
* New: A new frm_payment_user_id filter has been added for setting which user a payment belongs to, making it possible for an add-on to claim a payment for someone who is not logged in yet.
...
* Performance: Pricing updates for Stripe payments are now debounced, and are skipped on pages that have no payment intents, to reduce the number of server requests.
* Fix: A subscription would not get cancelled when it reached the limit set in the Recurring Payment Limit setting, since the customer check would always fail while a webhook event was being processed. A message now gets logged when a subscription cannot be cancelled.
* Fix: Payment status actions could run twice, because a Stripe Link return and a webhook event can both process the same payment.
...
* Fix: Conditionally hidden inputs would become visible after a card was declined when AJAX submit was enabled.
* Fix: When verifying a buyer with Square, the amount would not match the one expected on the server, and would always come from the first Square action even when conditional logic meant another should run.
