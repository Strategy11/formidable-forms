		...
		<div class="postbox">
			...
			<div class="frm_modal_footer">
				...
				if ( ! empty( $error_args['continue_text'] ) ) :
					?>
					<a href="<?php echo esc_url( $error_args['continue_url'] ); ?>" class="button button-primary dismiss frm-button-primary <?php echo esc_attr( $error_args['continue_classes'] ); ?>" <?php echo ! empty( $error_args['continue_target'] ) ? 'target="' . esc_attr( $error_args['continue_target'] ) . '" rel="noopener noreferrer"' : ''; ?>><?php echo esc_html( $error_args['continue_text'] ); ?></a>
				<?php endif; ?>
			</div>
		</div>
	</div>
