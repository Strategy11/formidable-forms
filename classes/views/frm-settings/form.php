						...
						<div id="contextual-help-columns">
							<div class="contextual-help-tabs">
								<ul class="frm-category-tabs">
									...
									<li <?php echo ( $a == 'general_settings' ) ? 'class="tabs active"' : ''; ?>>
										<a href="#general_settings" class="frm_cursor_pointer">
											<?php esc_html_e( 'General', 'formidable' ); ?>
										</a>
									</li>
							...
							<form name="frm_settings_form" method="post" class="frm_settings_form"
								...
								<div class="general_settings tabs-panel <?php echo esc_attr( $a === 'general_settings' ? 'frm_block' : 'frm_hidden' ); ?>">
									...
									<p>
										<label class="frm_left_label"><?php esc_html_e( 'Load form styling', 'formidable' ); ?></label>
									...
									<p>
										<label for="frm_use_html">
											<input type="checkbox" id="frm_use_html" name="frm_use_html"
													value="1" <?php checked( $frm_settings->use_html, 1 ); ?>> <?php esc_html_e( 'Use HTML5 in forms', 'formidable' ); ?>
										</label>
									...
									<p>
										<label for="frm_old_css">
											<input type="checkbox" id="frm_old_css" name="frm_old_css"
													value="1" <?php checked( $frm_settings->old_css, 1 ); ?>>
											<?php esc_html_e( 'Do not use CSS Grids for form layouts', 'formidable' ); ?>
										</label>
										<span class="frm_help frm_icon_font frm_tooltip_icon"
												title="<?php esc_attr_e( 'Form layouts built using CSS grids that are not fully supported by older browsers like Internet Explorer. Leave this box unchecked for your layouts to look best in current browsers, but show in a single column in older browsers.', 'formidable' ); ?>"></span>
									</p>
									...
									<h3><?php esc_html_e( 'User Permissions', 'formidable' ); ?>
										<span class="frm_help frm_icon_font frm_tooltip_icon"
												title="<?php esc_attr_e( 'Select users that are allowed access to Formidable. Without access to View Forms, users will be unable to see the Formidable menu.', 'formidable' ); ?>"></span>
									</h3>
										...
										foreach ( $frm_roles as $frm_role => $frm_role_description ) {
											...
											<tr>
												<td class="frm_left_label">
													<label><?php echo esc_html( $frm_role_description ); ?></label></td>
									...
									<h3><?php esc_html_e( 'reCAPTCHA', 'formidable' ); ?>
										<span class="frm_help frm_icon_font frm_tooltip_icon"
												title="<?php esc_attr_e( 'reCAPTCHA is a free, accessible CAPTCHA service that helps to digitize books while blocking spam on your blog. reCAPTCHA asks commenters to retype two words scanned from a book to prove that they are a human. This verifies that they are not a spambot.', 'formidable' ); ?>"></span>
									</h3>
									...
									<p>
										<label class="frm_left_label"><?php esc_html_e( 'Site Key', 'formidable' ); ?></label>
										<input type="text" name="frm_pubkey" id="frm_pubkey" size="42"
												value="<?php echo esc_attr( $frm_settings->pubkey ); ?>"
												placeholder="<?php esc_attr_e( 'Optional', 'formidable' ); ?>"/></p>

									<p>
										<label class="frm_left_label"><?php esc_html_e( 'Secret Key', 'formidable' ); ?></label>
										<input type="text" name="frm_privkey" id="frm_privkey" size="42"
												value="<?php echo esc_attr( $frm_settings->privkey ); ?>"
												placeholder="<?php esc_attr_e( 'Optional', 'formidable' ); ?>"/></p>

									<p>
										<label class="frm_left_label"><?php esc_html_e( 'reCAPTCHA Type', 'formidable' ); ?></label>
									...
									<p>
										<label class="frm_left_label"><?php esc_html_e( 'reCAPTCHA Language', 'formidable' ); ?></label>
									...
									<h3><?php esc_html_e( 'Default Messages', 'formidable' ); ?>
										<span class="frm_help frm_icon_font frm_tooltip_icon"
												title="<?php esc_attr_e( 'You can override the success message and submit button settings on individual forms.', 'formidable' ); ?>"></span>
									</h3>
									...
									<?php if ( ! FrmAppHelper::pro_is_installed() ) { ?>
										...
										<input type="hidden" name="frm_menu" id="frm_menu"
												value="<?php echo esc_attr( $frm_settings->menu ); ?>"/>
										<input type="hidden" name="frm_mu_menu" id="frm_mu_menu"
												value="<?php echo esc_attr( $frm_settings->mu_menu ); ?>"/>
									<?php } ?>

									<p>
										<label class="frm_left_label"><?php esc_html_e( 'IP storage', 'formidable' ); ?></label>
										<label for="frm_no_ips">
											<input type="checkbox" name="frm_no_ips" id="frm_no_ips"
													value="1" <?php checked( $frm_settings->no_ips, 1 ); ?> />
