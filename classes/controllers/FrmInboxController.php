...
class FrmInboxController {
	...
	private static function add_tracking_request() {
		...
		$message->add_message(
			array(
				'key'     => 'usage',
				'message' => 'Gathering usage data allows us to improve Formidable. Your forms will be considered as we evaluate new features, judge the quality of an update, or determine if an improvement makes sense. You can always visit the <a href="' . esc_url( $link ) . '">Global Settings</a> and choose to stop sharing data. <a href="' . esc_url( FrmAppHelper::get_doc_url( 'global-settings-overview/#kb-usage-tracking', 'usage-tracking-inbox' ) ) . '" target="_blank" rel="noopener noreferrer">Read more about what data we collect</a>.', // phpcs:ignore SlevomatCodingStandard.Files.LineLength.LineTooLong
