...
class FrmFieldsController {
	...
	/**
	 * Get an instance of the FrmField object or one of its children, depending on a field's type
	 *
	 * @since 2.03.05
	 *
	 * @param string $id_or_key
	 *
	 * @return FrmFieldText|FrmFieldParagraph|FrmFieldCheckbox|FrmFieldRadio|FrmFieldDropdown|
	 * FrmEmailField|FrmWebsiteField|FrmReCaptchaField
	 */
    public static function get_field( $id_or_key ) {
    	$field = null;

	    $id = self::get_field_id( $id_or_key );

	    if ( ! $id ) {
	        return $field;
	    }

	    $type = FrmField::get_type( $id );

	    if ( ! $type ) {
		    return $field;
	    }

	    $field = apply_filters( 'frm_instantiate_field_object', $field, $id, $type );

	    if ( is_object( $field ) ) {
	    	// Do not change object if it was instantiated in pro code
	    } else {
	    	$field = self::get_free_field( $id, $type );
	    }
	...
	/**
	 * Get the field ID from a key or ID
	 *
	 * @since 2.03.05
	 *
	 * @param string $id_or_key
	 ...
	 * @return string
	 */
    private static function get_field_id( $id_or_key ) {
	    if ( $id_or_key == '' ) {
		    $id = '';
	    } else if ( is_numeric( $id_or_key ) ) {
		    $id = $id_or_key;
	    } else {
		    $id = FrmField::get_id_by_key( $id_or_key );
	...
	/**
	 * Get an instance of a free field object
	 *
	 * @since 2.03.05
	 *
	 * @param string $id
	 ...
	 *
	 * @return FrmFieldText|FrmFieldParagraph|FrmFieldCheckbox|FrmFieldRadio|FrmFieldDropdown|
	 * FrmEmailField|FrmWebsiteField|FrmReCaptchaField|FrmFieldGeneric
	 */
    private static function get_free_field( $id, $type ) {
	    if ( $type === 'text' ) {
		    $field = new FrmFieldText( $id );
	    } elseif ( $type === 'textarea' ) {
		    $field = new FrmFieldParagraph( $id );
	    } elseif ( $type === 'checkbox' ) {
		    $field = new FrmFieldCheckbox( $id );
	    } elseif ( $type === 'radio' ) {
		    $field = new FrmFieldRadio( $id );
	    } elseif ( $type === 'select' ) {
		    $field = new FrmFieldDropdown( $id );
	    } elseif ( $type === 'email' ) {
		    $field = new FrmFieldEmail( $id );
	    } else if ( $type === 'url' ) {
		    $field = new FrmFieldWebsite( $id );
	    } elseif ( $type === 'captcha' ) {
		    $field = new FrmFieldReCaptcha( $id );
	    } else {
	    	$field = new FrmFieldGeneric( $id );
	    }
