...
class FrmStylesCardHelper {
	...
	public static function get_style_param_for_card( $style ) {
		...
		if ( empty( $style->post_content['fieldset_bg_color'] ) ) {
			...
			$bg_color = $style->post_content['fieldset_bg_color'];
			$background_color = str_starts_with( $bg_color, 'rgb' ) ? $bg_color : ( str_starts_with( $bg_color, '#' ) ? $bg_color : '#' . $bg_color ); // phpcs:ignore SlevomatCodingStandard.Files.LineLength.LineTooLong
		}
