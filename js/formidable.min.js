...
hideBy:a.hideBy});else{var d="none";if("show"==a.Show){if(!0!==l[a.HideField][b]){I(l[a.HideField][b],a.FieldName,c);return}d=""}"."===a.hideBy?(b=jQuery("."+a.hideContainerID),b.length&&("none"===d?w(b,a):b.show())):(b=document.getElementById(a.hideContainerID),null!==b&&("none"==d?w(jQuery(b),a):b.style.display=d))}}function P(b){jQuery.each(D,function(a,c){if("undefined"!=typeof c&&"undefined"!=typeof c.result){var d=jQuery(c.hideBy+c.hideContainerID),e=c.show;if(d.length){if("any"==c.match&&-1==
jQuery.inArray(!0,l[c.HideField])||"all"==c.match&&-1<jQuery.inArray(!1,l[c.HideField]))e="show"==c.show?"hide":"show";"show"==e?d.show():w(d,c);!1!==typeof c.result&&!0!==typeof c.result&&I(c.result,c.FieldName,b)}delete D[a]}})}function y(b,a,c){"undefined"==typeof c&&(c="");jQuery.isArray(c)&&-1<jQuery.inArray(a,c)&&(c=a);-1!=String(a).search(/^\s*(\+|-)?((\d+(\.\d+)?)|(\.\d+))\s*$/)&&(a=parseFloat(a),c=parseFloat(c));return"-1"!=String(a).indexOf("&quot;")&&y(b,a.replace("&quot;",'"'),c)?!0:{"==":function(b,
a){return b==a},"!=":function(b,a){return b!=a},"<":function(b,a){return b>a},">":function(b,a){return b<a},LIKE:function(b,a){return a?-1!=a.indexOf(b):0},"not LIKE":function(b,a){return a?-1==a.indexOf(b):1}}[b](a,c)}function I(b,a,c){"getDataOpts"==b.funcName?R(b.f,b.sel,a,c):"getData"==b.funcName&&H(b.f,b.sel,0)}function H(b,a,c){var d=document.getElementById(b.hideContainerID),e=jQuery(d).find(".frm_data_field_container");if(0===e.length)return!0;c||e.html('<span class="frm-loading-img"></span>');
jQuery.ajax({type:"POST",url:frm_js.ajax_url,data:{action:"frm_fields_ajax_get_data",entry_id:a,field_id:b.LinkedField,current_field:b.HideField,hide_id:b.hideContainerID,nonce:frm_js.nonce},success:function(f){""!==f&&(d.style.display="");if(c)e.append(f);else{e.html(f);var h=e.children("input"),k=h.val();if(""===f||""===k)d.style.display="none";p(a,b.HideField,null,h)}return!0}})}function R(b,a,c,d){if(!("stop"==d&&-1<jQuery.inArray(b.HideField,E)&&b.parentField&&"hidden"==b.parentField.attr("type"))){var e=
jQuery('input[name^="'+b.hiddenName+'"], select[name^="'+b.hiddenName+'"]:not(":disabled"), textarea[name^="'+b.hiddenName+'"]'),f=[];e.each(function(){"radio"==this.type||"checkbox"==this.type?!0===this.checked&&f.push(jQuery(this).val()):f.push(jQuery(this).val())});if("select"!=b.DataType||"stop"!=d&&!jQuery("#"+b.hideContainerID+" .frm-loading-img").length||!(-1<jQuery.inArray(b.HideField,E))){0===f.length&&(f="");E.push(b.HideField);var h=document.getElementById(b.hideContainerID),k=jQuery(h).find(".frm_data_field_container");
...
a?(h.style.display="none",f=""):"all"!=b.MatchType&&(h.style.display="");k.html(a);var c=k.find("select, input, textarea");""!==a&&""!==f&&(jQuery.isArray(f)||(a=[],a.push(f),f=a),jQuery.each(f,function(a,b){if("undefined"!=typeof b&&""!==b)if("checkbox"==g||"radio"==g)1<c.length?c.filter('[value="'+b+'"]').attr("checked","checked"):c.val()==b&&c.attr("checked","checked");else if("select"==g){var d=c.children('option[value="'+b+'"]');d.length?d.prop("selected",!0):f.splice(a,1)}else c.val(b)}));c.hasClass("frm_chzn")&&
