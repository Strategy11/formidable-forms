...
function M(b,a,c,d){if(!("stop"==d&&-1<jQuery.inArray(b.HideField,E)&&"hidden"==b.parentField.attr("type"))){var e=jQuery('input[name^="'+b.hiddenName+'"], select[name^="'+b.hiddenName+'"], textarea[name^="'+b.hiddenName+'"]'),f=[];e.each(function(){"radio"==this.type||"checkbox"==this.type?!0===this.checked&&f.push(jQuery(this).val()):f.push(jQuery(this).val())});if("select"!=b.DataType||"stop"!=d&&!jQuery("#"+b.hideContainerID+" .frm-loading-img").length||!(-1<jQuery.inArray(b.HideField,E))){0===
f.length&&(f="");E.push(b.HideField);var h=document.getElementById(b.hideContainerID),n=jQuery(h).find(".frm_data_field_container");if(0===n.length&&e.length)return p(f,b.HideField,"stop",e),!1;if(""!==b.Value&&!y(b.Condition,b.Value,a))return h.style.display="none",n.html(""),p("",b.HideField,"stop",e),!1;n.html('<span class="frm-loading-img" style="visibility:visible;display:inline;"></span>');var q=b.DataType;jQuery.ajax({type:"POST",url:frm_js.ajax_url,data:{action:"frm_fields_ajax_data_options",
...
e[k])}q=q.calc_dec;l.indexOf(").toFixed(")&&(g=l.split(").toFixed("),H(g[1])&&(q=g[1],l=l.replace(").toFixed("+q,"")));l=parseFloat(eval(l));H(q)&&(l=l.toFixed(q));"undefined"===typeof l&&(l=0);jQuery(document.getElementById("field_"+d[h])).val(l).trigger({type:"change",frmTriggered:d[h],selfTriggered:!0})}}}}function N(b,a,c,d,e){if("undefined"!==typeof e[a]&&0!==e[a])return e[a];jQuery(b).each(function(){"undefined"===typeof e[a]&&(e[a]=0);var b;a:{if("checkbox"==c.type||"radio"==c.type){if(b=c.key.replace('[id^="',
""),b=b.replace('"]',""),-1<this.id.replace(b,"").indexOf("other")){b=!0;break a}}else if("select"==c.type&&(b=this.className)&&-1<b.indexOf("frm_other_trigger")){b=!0;break a}b=!1}b?(b=0,"checkbox"==c.type||"radio"==c.type?b=jQuery(this).closest(".frm_"+c.type).children(".frm_other_input").val():"select"==c.type&&(b=jQuery(this).closest(".frm_other_container").children(".frm_other_input").val()),"undefined"===typeof b&&(b=0)):b=jQuery(this).val();if("date"==c.type){var h=jQuery.datepicker.parseDate(d.date,
...
b+"-","")),"undefined"==typeof a&&(a=1));jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"json",data:{action:"frm_add_form_row",field_id:b,i:a,nonce:frm_js.nonce},success:function(a){var d=a.html,e=jQuery(d).hide().fadeIn("slow");jQuery(".frm_repeat_"+b+":last").after(e);var f=["other"],h,g="reset";frmFrontForm.hideCondFields(JSON.stringify(a.logic.hide));jQuery(d).find("input, select, textarea").each(function(){"file"!=this.type&&(h=this.name.replace("item_meta[","").split("]")[2].replace("[",
""),-1==jQuery.inArray(h,f)&&(f.push(h),p("und",h,null,jQuery(this),g),B("",h),g="persist"))});for(var e=0,k=a.logic.check.length;e<k;e++)-1==jQuery.inArray(a.logic.check[e],f)&&1>jQuery(d).find(".frm_field_"+a.logic.check[e]+"_container").length&&(p("und",a.logic.check[e],null,null,g),g="persist");0<jQuery(d).find(".star").length&&jQuery(".star").rating();0<jQuery(d).find(".frm_chzn").length&&jQuery().chosen&&jQuery(".frm_chzn").chosen({allow_single_deselect:!0});"function"==typeof frmThemeOverride_frmAddRow&&
frmThemeOverride_frmAddRow(b,a)}});return!1}function U(b){b.fadeOut("slow",function(){b.remove()})}function Y(){var b=jQuery(this).data("frmtoggle");jQuery(b).is(":visible")?jQuery(b).slideUp("fast"):jQuery(b).slideDown("fast");return!1}function H(b){return!jQuery.isArray(b)&&0<=b-parseFloat(b)+1}function L(b,a){var c=[];if("undefined"==typeof document.querySelector)jQuery("#"+b+' input[type=checkbox]:checked, input[type=hidden][name^="'+a+'"]').each(function(){c.push(this.value)});else for(var d=
document.querySelectorAll("#"+b+' input[type=checkbox], input[type=hidden][name^="'+a+'"]'),e=0;e<d.length;e++)("checkbox"==d[e].type&&d[e].checked||"hidden"==d[e].type)&&c.push(d[e].value);return c}var k=[],D=[],E=[];return{init:function(){jQuery(document).on("click",".frm_trigger",m);var b=jQuery(".frm_blank_field");b.length&&b.closest(".frm_toggle_container").prev(".frm_trigger").click();jQuery.isFunction(jQuery.fn.placeholder)?jQuery(".frm-show-form input, .frm-show-form textarea").placeholder():
jQuery(".frm-show-form input[onblur], .frm-show-form textarea[onblur]").each(function(){""===jQuery(this).val()&&jQuery(this).blur()});jQuery(document).on("focus",".frm_toggle_default",P);jQuery(document).on("blur",".frm_toggle_default",Q);jQuery(".frm_toggle_default").blur();jQuery(document.getElementById("frm_resend_email")).click(R);jQuery(document).on("change",".frm_multiple_file",S);jQuery(document).on("click",".frm_clear_file_link",V);jQuery(document).on("click",".frm_remove_link",T);jQuery(document).on("focusin",
