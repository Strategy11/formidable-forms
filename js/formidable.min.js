...
"\r");if(""===c||"undefined"==typeof c)return!1;var d=b.val().replace(/(\n|\r\n)/g,"\r");"replace"==a?""===d&&b.addClass("frm_default").val(c):d==c&&b.removeClass("frm_default").val("")}function V(){var b=jQuery(this),a=b.data("eid"),c=b.data("fid");b.append('<span class="spinner" style="display:inline"></span>');jQuery.ajax({type:"POST",url:frm_js.ajax_url,data:{action:"frm_entries_send_email",entry_id:a,form_id:c,nonce:frm_js.nonce},success:function(a){b.replaceWith(a)}});return!1}function M(b){var a=
"string";if("number"==b.type)a="number";else if("checkbox"==b.type||"select"==b.type){var c=b.options.length;"select"==b.type&&""===b.options[0]&&(c="post_status"==b.field_options.post_field?3:c-1);1==c&&(a="boolean")}return a}function W(b){var a=new google.visualization.DataTable,c=!1,d=!1,e=b.rows.length;if(0<e)if("table"==b.type){c=!0;a.addRows(b.rows[e-1][0]+1);for(var f=0;f<e;f++)a.setCell(b.rows[f])}else if("undefined"!=typeof b.rows[0].tooltip)for(var d=c=!0,h=0;h<e;h++){f=b.rows[h].tooltip;
delete b.rows[h].tooltip;var k=Object.keys(b.rows[h]).map(function(a){return b.rows[h][a]});b.rows[h]=k;b.rows[h].push(f)}e=b.cols.length;if(c){if(0<e)for(c=0;c<e;c++)f=b.cols[c],a.addColumn(f.type,f.name);d&&(a.addColumn({type:"string",role:"tooltip"}),a.addRows(b.rows))}else{a=[[]];for(d=0;d<e;d++)a[0].push(b.cols[d].name);a=a.concat(b.rows);a=google.visualization.arrayToDataTable(a)}d=b.type.charAt(0).toUpperCase()+b.type.slice(1)+"Chart";(new google.visualization[d](document.getElementById("chart_"+
