...
selfTriggered:!0});d=!0})}}}function A(b,a,c){if("all"==a.MatchType||!1===l[a.HideField][b])D.push({result:l[a.HideField][b],show:a.Show,match:a.MatchType,FieldName:a.FieldName,HideField:a.HideField,hideContainerID:a.hideContainerID,hideBy:a.hideBy});else{var d="none";if("show"==a.Show){if(!0!==l[a.HideField][b]){H(l[a.HideField][b],a.FieldName,c);return}d=""}"."===a.hideBy?(b=jQuery("."+a.hideContainerID),b.length&&("none"===d?w(b,a):b.show())):(b=document.getElementById(a.hideContainerID),null!==
b&&("none"==d?w(jQuery(b),a):b.style.display=d))}}function P(b){jQuery.each(D,function(a,c){if("undefined"!=typeof c&&"undefined"!=typeof c.result){var d=jQuery(c.hideBy+c.hideContainerID),e=c.show;if(d.length){if("any"==c.match&&-1==jQuery.inArray(!0,l[c.HideField])||"all"==c.match&&-1<jQuery.inArray(!1,l[c.HideField]))e="show"==c.show?"hide":"show";"show"==e?d.show():w(d,c);!1!==typeof c.result&&!0!==typeof c.result&&H(c.result,c.FieldName,b)}delete D[a]}})}function y(b,a,c){"undefined"==typeof c&&
(c="");jQuery.isArray(c)&&-1<jQuery.inArray(a,c)&&(c=a);-1!=String(a).search(/^\s*(\+|-)?((\d+(\.\d+)?)|(\.\d+))\s*$/)&&(a=parseFloat(a),c=parseFloat(c));return"-1"!=String(a).indexOf("&quot;")&&y(b,a.replace("&quot;",'"'),c)?!0:{"==":function(b,a){return b==a},"!=":function(b,a){return b!=a},"<":function(b,a){return b>a},">":function(b,a){return b<a},LIKE:function(b,a){return a?-1!=a.indexOf(b):0},"not LIKE":function(b,a){return a?-1==a.indexOf(b):1}}[b](a,c)}function H(b,a,c){"getDataOpts"==b.funcName?
R(b.f,b.sel,a,c):"getData"==b.funcName&&G(b.f,b.sel,0)}function G(b,a,c){var d=document.getElementById(b.hideContainerID),e=jQuery(d).find(".frm_data_field_container");if(0===e.length)return!0;c||e.html('<span class="frm-loading-img"></span>');jQuery.ajax({type:"POST",url:frm_js.ajax_url,data:{action:"frm_fields_ajax_get_data",entry_id:a,field_id:b.LinkedField,current_field:b.HideField,hide_id:b.hideContainerID,nonce:frm_js.nonce},success:function(f){""!==f&&(d.style.display="");if(c)e.append(f);
else{e.html(f);var g=e.children("input"),k=g.val();if(""===f||""===k)d.style.display="none";p(a,b.HideField,null,g)}return!0}})}function R(b,a,c,d){if(!("stop"==d&&-1<jQuery.inArray(b.HideField,E)&&b.parentField&&"hidden"==b.parentField.attr("type"))){var e=jQuery('input[name^="'+b.hiddenName+'"], select[name^="'+b.hiddenName+'"]:not(":disabled"), textarea[name^="'+b.hiddenName+'"]'),f=[];e.each(function(){"radio"==this.type||"checkbox"==this.type?!0===this.checked&&f.push(jQuery(this).val()):f.push(jQuery(this).val())});
if("select"!=b.DataType||"stop"!=d&&!jQuery("#"+b.hideContainerID+" .frm-loading-img").length||!(-1<jQuery.inArray(b.HideField,E))){0===f.length&&(f="");E.push(b.HideField);var g=document.getElementById(b.hideContainerID),k=jQuery(g).find(".frm_data_field_container");if(0===k.length&&e.length)return p(f,b.HideField,"stop",e),!1;if(""!==b.Value&&!y(b.Condition,b.Value,a))return g.style.display="none",k.html(""),p("",b.HideField,"stop",e),!1;k.html('<span class="frm-loading-img" style="visibility:visible;display:inline;"></span>');
var h=b.DataType;jQuery.ajax({type:"POST",url:frm_js.ajax_url,data:{action:"frm_fields_ajax_data_options",hide_field:c,entry_id:a,selected_field_id:b.LinkedField,field_id:b.HideField,hide_id:b.hideContainerID,nonce:frm_js.nonce},success:function(a){""===a?(g.style.display="none",f=""):"all"!=b.MatchType&&(g.style.display="");k.html(a);var c=k.find("select, input, textarea");""!==a&&""!==f&&(jQuery.isArray(f)||(a=[],a.push(f),f=a),jQuery.each(f,function(a,b){if("undefined"!=typeof b&&""!==b)if("checkbox"==
...
b.total;for(var c=[],d=0,e=0,f=b.length;e<f;e++){for(var g=a.calc[b[e]],k=g.calc,d=g.fields.length,h=0;h<d;h++){var m=g.fields[h],l=a.fields[m],n="input"+a.fieldKeys[m];"checkbox"==l.type||"select"==l.type?n=n+":checked,select"+a.fieldKeys[m]+" option:selected,"+n+"[type=hidden]":"radio"==l.type||"scale"==l.type?n=n+":checked,"+n+"[type=hidden]":"textarea"==l.type&&(n=n+",textarea"+a.fieldKeys[m]);c[m]=S(n,m,l,a,c);if("undefined"===typeof c[m]||isNaN(c[m]))c[m]=0;l="["+m+"]";l=l.replace(/([.*+?^=!:${}()|\[\]\/\\])/g,
"\\$1");k=k.replace(new RegExp(l,"g"),c[m])}g=g.calc_dec;k.indexOf(").toFixed(")&&(h=k.split(").toFixed("),I(h[1])&&(g=h[1],k=k.replace(").toFixed("+g,"")));k=parseFloat(eval(k));I(g)&&(k=k.toFixed(g));"undefined"===typeof k&&(k=0);jQuery(document.getElementById("field_"+b[e])).val(k).trigger({type:"change",frmTriggered:b[e],selfTriggered:!0})}}}}function S(b,a,c,d,e){if("undefined"!==typeof e[a]&&0!==e[a])return e[a];jQuery(b).each(function(){"undefined"===typeof e[a]&&(e[a]=0);var b;b=!1;if("hidden"==
...
b.type||"select"==b.type){var c=b.options.length;"select"==b.type&&""===b.options[0]&&(c="post_status"==b.field_options.post_field?3:c-1);1==c&&(a="boolean")}return a}function W(b){var a=new google.visualization.DataTable,c=!1,d=!1,e=b.rows.length;if(0<e)if("table"==b.type){c=!0;a.addRows(b.rows[e-1][0]+1);for(var f=0;f<e;f++)a.setCell(b.rows[f])}else if("undefined"!=typeof b.rows[0].tooltip)for(var d=c=!0,g=0;g<e;g++){f=b.rows[g].tooltip;delete b.rows[g].tooltip;var k=Object.keys(b.rows[g]).map(function(a){return b.rows[g][a]});
b.rows[g]=k;b.rows[g].push(f)}e=b.cols.length;if(c){if(0<e)for(c=0;c<e;c++)f=b.cols[c],a.addColumn(f.type,f.name);d&&(a.addColumn({type:"string",role:"tooltip"}),a.addRows(b.rows))}else{a=[[]];for(d=0;d<e;d++)a[0].push(b.cols[d].name);a=a.concat(b.rows);a=google.visualization.arrayToDataTable(a)}d=b.type.charAt(0).toUpperCase()+b.type.slice(1)+"Chart";(new google.visualization[d](document.getElementById("chart_"+b.graph_id))).draw(a,b.options)}function X(){var b=jQuery(this),a=b.data("fid");b.wrap('<div class="frm_file_names frm_uploaded_files">');
...
function aa(){jQuery(this).parent(".frm_file_names").replaceWith("");return!1}function ba(){var b="frm_section_"+jQuery(this).data("parent")+"-"+jQuery(this).data("key"),a=jQuery(document.getElementById(b)),c=a.find("input, select, textarea");a.fadeOut("slow",function(){a.remove();c.each(function(){if("file"!=this.type){var a=this.name.replace("item_meta[","").split("]")[2].replace("[","");C(a)}})});return!1}function ca(){var b=jQuery(this).data("parent"),a=0;0<jQuery(".frm_repeat_"+b).length&&(a=
1+parseInt(jQuery(".frm_repeat_"+b+":last").attr("id").replace("frm_section_"+b+"-","")),"undefined"==typeof a&&(a=1));jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"json",data:{action:"frm_add_form_row",field_id:b,i:a,nonce:frm_js.nonce},success:function(a){var d=a.html,e=jQuery(d).hide().fadeIn("slow");jQuery(".frm_repeat_"+b+":last").after(e);var f=["other"],g,k="reset";jQuery(d).find("input, select, textarea").each(function(){"file"!=this.type&&(g=this.name.replace("item_meta[","").split("]")[2].replace("[",
""),-1==jQuery.inArray(g,f)&&(f.push(g),p("und",g,null,jQuery(this),k),C(g),k="persist"))});for(var e=0,h=a.logic.check.length;e<h;e++)-1==jQuery.inArray(a.logic.check[e],f)&&1>jQuery(d).find(".frm_field_"+a.logic.check[e]+"_container").length&&(p("und",a.logic.check[e],null,null,k),k="persist");0<jQuery(d).find(".star").length&&jQuery(".star").rating();0<jQuery(d).find(".frm_chzn").length&&jQuery().chosen&&jQuery(".frm_chzn").chosen({allow_single_deselect:!0});"function"==typeof frmThemeOverride_frmAddRow&&
frmThemeOverride_frmAddRow(b,a)}});return!1}function Z(b){b.fadeOut("slow",function(){b.remove()})}function da(){var b=jQuery(this).data("frmtoggle");jQuery(b).is(":visible")?jQuery(b).slideUp("fast"):jQuery(b).slideDown("fast");return!1}function I(b){return!jQuery.isArray(b)&&0<=b-parseFloat(b)+1}function Q(b,a){var c=[];if("undefined"==typeof document.querySelector)jQuery("#"+b+' input[type=checkbox]:checked, input[type=hidden][name^="'+a+'"]').each(function(){c.push(this.value)});else for(var d=
document.querySelectorAll("#"+b+' input[type=checkbox], input[type=hidden][name^="'+a+'"]'),e=0;e<d.length;e++)("checkbox"==d[e].type&&d[e].checked||"hidden"==d[e].type)&&c.push(d[e].value);return c}var l=[],D=[],z=[],E=[],F="",B=[];return{init:function(){jQuery(document).on("click",".frm_trigger",n);var b=jQuery(".frm_blank_field");b.length&&b.closest(".frm_toggle_container").prev(".frm_trigger").click();jQuery.isFunction(jQuery.fn.placeholder)?jQuery(".frm-show-form input, .frm-show-form textarea").placeholder():
jQuery(".frm-show-form input[onblur], .frm-show-form textarea[onblur]").each(function(){""===jQuery(this).val()&&jQuery(this).blur()});jQuery(document).on("focus",".frm_toggle_default",T);jQuery(document).on("blur",".frm_toggle_default",U);jQuery(".frm_toggle_default").blur();jQuery(document.getElementById("frm_resend_email")).click(V);jQuery(document).on("change",".frm_multiple_file",X);jQuery(document).on("click",".frm_clear_file_link",aa);jQuery(document).on("click",".frm_remove_link",Y);jQuery(document).on("focusin",
...
frm_js.id));for(var f=c.fields.length,g="string",h=0,l=f;h<l;h++){var m=c.fields[h],g=M(m);d.addColumn(g,m.name)}h=!1;c.options.edit_link&&(h=!0,d.addColumn("string",c.options.edit_link));l=!1;c.options.delete_link&&(l=!0,d.addColumn("string",c.options.delete_link));m=0;if(null!==c.entries){var n=c.entries.length;d.addRows(n);for(var p=0,r=0;r<n;r++){var m=0,q=c.entries[r];e&&(d.setCell(p,m,q.id),m++);for(var t=0,u=f;t<u;t++){var v=c.fields[t],g=M(v),v=q.metas[v.id];"number"!=g||null!==v&&""!==v?
"boolean"==g&&(v=null===v||"false"==v||!1===v?!1:!0):v=0;d.setCell(p,m,v);m++}h&&("undefined"!==typeof q.editLink?d.setCell(p,m,'<a href="'+q.editLink+'">'+c.options.edit_link+"</a>"):d.setCell(p,m,""),m++);l&&("undefined"!==typeof q.deleteLink?d.setCell(p,m,'<a href="'+q.deleteLink+'" class="frm_delete_link" onclick="return confirm('+c.options.confirm+')">'+c.options.delete_link+"</a>"):d.setCell(p,m,""));p++}}else for(d.addRows(1),h=m=0,l=f;h<l;h++)0<m?d.setCell(0,m,""):d.setCell(0,m,c.options.no_entries),
