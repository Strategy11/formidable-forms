...
"_container"));var h=a.offset();"undefined"!==typeof h&&(c=h.top);a.replaceWith(f);e=document.documentElement.scrollTop||document.body.scrollTop;c&&c>frm_js.offset&&e>c&&jQuery(window).scrollTop(c-frm_js.offset);if("function"==typeof frmThemeOverride_frmAfterSubmit){var h=jQuery(f).find('input[name="form_id"]').val(),p="";h&&(p=jQuery('input[name="frm_page_order_'+h+'"]').val());frmThemeOverride_frmAfterSubmit(h,p,f,b)}jQuery(b).find('input[name="id"]').length&&(f=jQuery(b).find('input[name="id"]').val(),
...
frm_js.nonce+'">');h&&b.submit()}},error:function(){jQuery(b).find('input[type="submit"], input[type="button"]').removeAttr("disabled");b.submit()}})}function R(){J(jQuery(this),"clear")}function S(){J(jQuery(this),"replace")}function J(b,a){var c=b.data("frmval").replace(/(\n|\r\n)/g,"\r");if(""===c||"undefined"==typeof c)return!1;var e=b.val().replace(/(\n|\r\n)/g,"\r");"replace"==a?""===e&&b.addClass("frm_default").val(c):e==c&&b.removeClass("frm_default").val("")}function T(){var b=jQuery(this),
...
k),k="persist"))});for(var f=0,m=a.logic.check.length;f<m;f++)-1==jQuery.inArray(a.logic.check[f],g)&&1>jQuery(e).find(".frm_field_"+a.logic.check[f]+"_container").length&&(q("und",a.logic.check[f],null,null,k),k="persist");0<jQuery(e).find(".star").length&&jQuery(".star").rating();0<jQuery(e).find(".frm_chzn").length&&jQuery().chosen&&jQuery(".frm_chzn").chosen({allow_single_deselect:!0});"function"==typeof frmThemeOverride_frmAddRow&&frmThemeOverride_frmAddRow(b,a)}});return!1}function L(b){b.fadeOut("slow",
function(){b.remove()})}function I(b){return!jQuery.isArray(b)&&0<=b-parseFloat(b)+1}function N(b,a){var c=[];if("undefined"==typeof document.querySelector)jQuery("#"+b+' input[type=checkbox]:checked, input[type=hidden][name^="'+a+'"]').each(function(){c.push(this.value)});else for(var e=document.querySelectorAll("#"+b+' input[type=checkbox], input[type=hidden][name^="'+a+'"]'),f=0;f<e.length;f++)("checkbox"==e[f].type&&e[f].checked||"hidden"==e[f].type)&&c.push(e[f].value);return c}var m=[],E=[],
F=[];return{init:function(){jQuery(document).on("click",".frm_trigger",n);var b=jQuery(".frm_blank_field");b.length&&b.closest(".frm_toggle_container").prev(".frm_trigger").click();jQuery.isFunction(jQuery.fn.placeholder)?jQuery(".frm-show-form input, .frm-show-form textarea").placeholder():jQuery(".frm-show-form input[onblur], .frm-show-form textarea[onblur]").each(function(){""===jQuery(this).val()&&jQuery(this).blur()});jQuery(document).on("focus",".frm_toggle_default",R);jQuery(document).on("blur",
