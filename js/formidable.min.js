...
function B(a,b,c){if("all"==b.MatchType||!1===l[b.HideField][a])E.push({result:l[b.HideField][a],show:b.Show,match:b.MatchType,fname:b.FieldName,fkey:b.HideField,hideContainerID:b.hideContainerID});else{var e="none";if("show"==b.Show){if(!0!==l[b.HideField][a]){H(l[b.HideField][a],b.FieldName,c);return}e=""}a=document.getElementById(b.hideContainerID);null!==a&&(a.style.display=e)}}function D(a){jQuery.each(E,function(b,c){if("undefined"!=typeof c&&"undefined"!=typeof c.result){var e=document.getElementById(c.hideContainerID),
...
"_container"));c=b.offset().top;b.replaceWith(f);e=document.documentElement.scrollTop||document.body.scrollTop;c&&c>frm_js.offset&&e>c&&jQuery(window).scrollTop(c-frm_js.offset);if("function"==typeof frmThemeOverride_frmAfterSubmit){var h=jQuery(f).find('input[name="form_id"]').val(),k="";h&&(k=jQuery('input[name="frm_page_order_'+h+'"]').val());frmThemeOverride_frmAfterSubmit(h,k,f,a)}jQuery(a).find('input[name="id"]').length&&(f=jQuery(a).find('input[name="id"]').val(),jQuery(document.getElementById("frm_edit_"+
f)).find("a").addClass("frm_ajax_edited").click())}else{jQuery(a).find('input[type="submit"], input[type="button"]').removeAttr("disabled");jQuery(a).find(".frm_ajax_loading").removeClass("frm_loading_now");h=!0;jQuery(".form-field").removeClass("frm_blank_field");jQuery(".form-field .frm_error").replaceWith("");b="";var k=!1,l=null,m;for(m in f)if(l=jQuery(a).find(jQuery(document.getElementById("frm_field_"+m+"_container"))),l.length&&l.is(":visible"))h=!1,""===b&&(frmFrontForm.scrollMsg(m,a),b=
"#frm_field_"+m+"_container"),jQuery(a).find("#frm_field_"+m+"_container .g-recaptcha").length&&(k=!0,grecaptcha.reset()),l.addClass("frm_blank_field"),"function"==typeof frmThemeOverride_frmPlaceError?frmThemeOverride_frmPlaceError(m,f):l.append('<div class="frm_error">'+f[m]+"</div>");else if("redirect"==m){window.location=f[m];return}!0!==k&&jQuery(a).find(".g-recaptcha").closest(".frm_form_field").replaceWith('<input type="hidden" name="recaptcha_checked" value="'+frm_js.nonce+'">');h&&a.submit()}},
error:function(){jQuery(a).find('input[type="submit"], input[type="button"]').removeAttr("disabled");a.submit()}})}function R(){J(jQuery(this),"clear")}function S(){J(jQuery(this),"replace")}function J(a,b){var c=a.data("frmval").replace(/(\n|\r\n)/g,"\r");if(""===c||"undefined"==typeof c)return!1;var e=a.val().replace(/(\n|\r\n)/g,"\r");"replace"==b?""===e&&a.addClass("frm_default").val(c):e==c&&a.removeClass("frm_default").val("")}function T(){var a=jQuery(this),b=a.data("eid"),c=a.data("fid");
a.append('<span class="spinner" style="display:inline"></span>');jQuery.ajax({type:"POST",url:frm_js.ajax_url,data:{action:"frm_entries_send_email",entry_id:b,form_id:c,nonce:frm_js.nonce},success:function(b){a.replaceWith(b)}});return!1}function K(a){var b="string";if("number"==a.type)b="number";else if("checkbox"==a.type||"select"==a.type){var c=a.options.length;"select"==a.type&&""===a.options[0]&&(c="post_status"==a.field_options.post_field?3:c-1);1==c&&(b="boolean")}return b}function U(){var a=
jQuery(this),b=a.data("fid");a.wrap('<div class="frm_file_names frm_uploaded_files">');for(var c=a.get(0).files,e=0;e<c.length;e++)1==c.length?a.after(c[e].name+' <a href="#" class="frm_clear_file_link">'+frm_js.remove+"</a>"):a.after(c[e].name+"<br/>");a.hide();c=a.attr("name");c!="item_meta["+b+"][]"&&c.replace("item_meta[","").replace("[]","").split("][");a.closest(".frm_form_field").find(".frm_uploaded_files:last").after('<input name="'+c+'" data-fid="'+b+'"class="frm_transparent frm_multiple_file" multiple="multiple" type="file" />')}
...
frm_js.id));for(var g=c.fields.length,h="string",k=0,l=g;k<l;k++){var m=c.fields[k],h=K(m);e.addColumn(h,m.name)}k=!1;c.options.edit_link&&(k=!0,e.addColumn("string",c.options.edit_link));l=!1;c.options.delete_link&&(l=!0,e.addColumn("string",c.options.delete_link));m=0;if(null!==c.entries){var n=c.entries.length;e.addRows(n);for(var r=0,t=0;t<n;t++){var m=0,v=c.entries[t];f&&(e.setCell(r,m,v.id),m++);for(var u=0,w=g;u<w;u++){var x=c.fields[u],h=K(x),x=v.metas[x.id];"number"!=h||null!==x&&""!==x?
"boolean"==h&&(x=null===x||"false"==x||!1===x?"false":"true"):x=0;e.setCell(r,m,x);m++}k&&(e.setCell(r,m,'<a href="'+v.editLink+'">'+c.options.edit_link+"</a>"),m++);l&&e.setCell(r,m,'<a href="'+v.deleteLink+'" class="frm_delete_link" onclick="return confirm('+c.options.confirm+')">'+c.options.delete_link+"</a>");r++}}else for(e.addRows(1),k=m=0,l=g;k<l;k++)0<m?e.setCell(0,m,""):e.setCell(0,m,c.options.no_entries),m++;(new google.visualization.Table(document.getElementById("frm_google_table_"+c.options.form_id))).draw(e,
JSON.stringify(c.graphOpts))}else{c=c[b][a];e=new google.visualization.DataTable;f=!1;g=c.rows.length;if(0<g)if("table"==c.type)for(f=!0,e.addRows(c.rows[g-1][0]+1),h=0;h<g;h++)e.setCell(c.rows[h]);else if("undefined"!=typeof c.rows[0].tooltip){f=!0;e.addColumn({type:"string",role:"tooltip"});for(h=0;h<g;h++)m=c.rows[h].tooltip,c.rows[h].tooltip=null,c.rows[h].push(m);e.addRows(c.rows)}g=c.cols.length;if(f){if(0<g)for(f=0;f<g;f++)h=c.cols[f],e.addColumn(h.type,h.name)}else{e=[[]];for(f=0;f<g;f++)e[0].push(c.cols[f].name);
e=e.concat(c.rows);e=google.visualization.arrayToDataTable(e)}f=c.type.charAt(0).toUpperCase()+c.type.slice(1)+"Chart";(new google.visualization[f](document.getElementById("chart_"+c.graph_id))).draw(e,c.options)}},removeUsedTimes:function(a,b){var c=jQuery(a).parents("form:first").find('input[name="id"]');jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"json",data:{action:"frm_fields_ajax_time_options",time_field:b,date_field:a.id,entry_id:c?c.val():"",date:jQuery(a).val(),nonce:frm_js.nonce},
success:function(a){var c=jQuery(document.getElementById(b));c.find("option").removeAttr("disabled");if(a&&""!==a)for(var g in a)c.find('option[value="'+g+'"]').attr("disabled","disabled")}})},escapeHtml:function(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")},invisible:function(a){jQuery(a).css("visibility","hidden")},visible:function(a){jQuery(a).css("visibility","visible")}}}var frmFrontForm=frmFrontFormJS();jQuery(document).ready(function(k){frmFrontForm.init()});
