...
"checkbox"===a&&(this.checked?jQuery(this).closest(".frm_checkbox").children(".frm_other_input").removeClass("frm_pos_none"):jQuery(this).closest(".frm_checkbox").children(".frm_other_input").addClass("frm_pos_none").val(""))}function y(a){var b=A(this);if(b&&"undefined"!==typeof b){var c="reset";if(a.frmTriggered){if(a.frmTriggered==b)return;c="persist"}t(b,null,jQuery(this),c);K(b,jQuery(this))}}function A(a,b){var c="",c=a instanceof jQuery?a.attr("name"):a.name,c=c.replace("item_meta[","").replace("[]",
"").split("]");if(1>c.length)return 0;var c=c.filter(function(a){return""!==a}),d=c[0],f=!1;if(1===c.length||"[form"==c[1])return d;jQuery('input[name="item_meta['+d+'][form]"]').length&&(d=c[2].replace("[",""),f=!0);"other"===d&&(d=f?c[3].replace("[",""):c[1].replace("[",""));!0===b&&(d=d+"-"+c[0]+"-"+c[1].replace("[",""));return d}function t(a,b,c,d){var f;if("undefined"===typeof __FRMRULES||"undefined"===typeof __FRMRULES[a])f=void 0;else{f=__FRMRULES[a];for(var e=[],g=0,k=f.length;g<k;g++){var h=
...
e.attr("id"),d=!0));e={};g=0;for(k=f.length;g<k;g++)if(m=h=f[g].HideField,q=e,q[m]=m in q?q[m]+1:0,f[g].FieldName===a?v(e[h],f[g],b,c):v(e[h],f[g],b),g===k-1){a:for(m in h=b,m=q=void 0,D){q=D[m];delete D[m];if("undefined"===typeof q)break a;var l=jQuery("#"+q.hideContainerID),p=q.show;if("any"===q.match&&-1===jQuery.inArray(!0,n[q.hideContainerID])||"all"===q.match&&-1<jQuery.inArray(!1,n[q.hideContainerID]))p="show"===q.show?"hide":"show";if("show"===p){if(S(l,q),l=h,0<q.DynamicInfoIndices.length)for(var p=
void 0,r=0;r<q.DynamicInfoIndices.length;r++)p=q.DynamicInfoIndices[r],T(n[q.hideContainerID][p],q.FieldName,l)}else U(l,q)}d&&(B="")}}}function v(a,b,c,d){b.inputName="item_meta["+b.FieldName+"]";b.hiddenName="item_meta["+b.HideField+"]";b.containerID="frm_field_"+b.FieldName+"_container";b.hideContainerID="frm_field_"+b.HideField+"_container";if(""!==B)d=B,L(b.FieldName)&&(b.inputName=M(b.FieldName,d),b.containerID=V(b.FieldName,d)),b.hideContainerID=V(b.HideField,d),d=x(b),z(b),H(a,b,d),I(a,b,
c);else{if("undefined"===typeof d||null===d)if(d=jQuery('input[name^="'+b.inputName+'"], textarea[name^="'+b.inputName+'"], select[name^="'+b.inputName+'"]'),1>d.length){d=document.getElementsByClassName("frm_field_"+b.FieldName+"_container");for(var f=0;f<d.length;f++){b.inputName=M(b.FieldName,d[f].id);b.containerID=d[f].id;b.hideContainerID=d[f].id.replace(b.FieldName,b.HideField);var e=x(b);z(b);H(a,b,e);I(a,b,c)}return}d=x(b);if(L(b.HideField))for(f=document.getElementsByClassName("frm_field_"+
b.HideField+"_container"),e=0;e<f.length;e++)b.hideContainerID=f[e].id,z(b),H(a,b,d),I(a,b,c);else z(b),H(a,b,d),I(a,b,c)}}function x(a){var b="";if("checkbox"===a.Type||"data-checkbox"===a.Type)return a=ja(a.containerID,a.inputName),b=a.length?a:"";b=jQuery('input[name="'+a.inputName+'"][type="hidden"]').val();if("undefined"!==typeof b)return b;b="radio"==a.Type||"data-radio"===a.Type?jQuery('input[name="'+a.inputName+'"]:checked').val():"select"===a.Type||"data-select"===a.Type?jQuery('select[name^="'+
...
function frmEditEntry(l,p,r,w,y,A){console.warn("DEPRECATED: function frmEditEntry in v2.0.13 use frmFrontForm.editEntry");var t=jQuery(document.getElementById("frm_edit_"+l)),v=t.html(),x=jQuery(document.getElementById(p+l)),z=x.html();x.html('<span class="frm-loading-img" id="'+p+l+'"></span><div class="frm_orig_content" style="display:none">'+z+"</div>");jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"html",data:{action:"frm_entries_edit_entry_ajax",post_id:r,entry_id:l,id:w,nonce:frm_js.nonce},
success:function(z){x.children(".frm-loading-img").replaceWith(z);t.replaceWith('<span id="frm_edit_'+l+'"><a onclick="frmCancelEdit('+l+",'"+p+"','"+frmFrontForm.escapeHtml(v)+"',"+r+","+w+",'"+A+'\')" class="'+A+'">'+y+"</a></span>")}})}
function frmCancelEdit(l,p,r,w,y,A){console.warn("DEPRECATED: function frmCancelEdit in v2.0.13 use frmFrontForm.cancelEdit");var t=jQuery(document.getElementById("frm_edit_"+l)),v=t.find("a"),x=v.html();v.hasClass("frm_ajax_edited")||(v=jQuery(document.getElementById(p+l)),v.children(".frm_forms").replaceWith(""),v.children(".frm_orig_content").fadeIn("slow").removeClass("frm_orig_content"));t.replaceWith('<a id="frm_edit_'+l+'" class="frm_edit_link '+A+'" href="javascript:frmEditEntry('+l+",'"+
p+"',"+w+","+y+",'"+frmFrontForm.escapeHtml(x)+"','"+A+"')\">"+r+"</a>")}
