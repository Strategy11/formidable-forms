...
" textarea"),c.length)){c.each(function(){u(b,a,a.FieldName,d,e,jQuery(this))});return}d=""}c=[];if("checkbox"==a.Type||"data-checkbox"==a.Type)c=O(a.containerID,a.inputName),d=c.length?c:"";m[a.HideField][b]=""===d||1>d.length?!1:{funcName:"getDataOpts",f:a,sel:d};if("checkbox"==a.Type||"data-checkbox"==a.Type&&"undefined"==typeof a.LinkedField)if(c=m[a.HideField][b]=!1,""!==d)for("!="==a.Condition&&(m[a.HideField][b]=!0),f=0;f<d.length;f++)c=y(a.Condition,a.Value,d[f]),"!="==a.Condition?!0===m[a.HideField][b]&&
...
(a=parseFloat(a),c=parseFloat(c));return"-1"!=String(a).indexOf("&quot;")&&y(b,a.replace("&quot;",'"'),c)?!0:{"==":function(a,b){return a==b},"!=":function(a,b){return a!=b},"<":function(a,b){return a>b},">":function(a,b){return a<b},LIKE:function(a,b){return b?-1!=b.indexOf(a):0},"not LIKE":function(a,b){return b?-1==b.indexOf(a):1}}[b](a,c)}function I(b,a,c){"getDataOpts"==b.funcName?P(b.f,b.sel,a,c):"getData"==b.funcName&&H(b.f,b.sel,0)}function H(b,a,c){var d=document.getElementById(b.hideContainerID),
...
(g=this.name.replace("item_meta[","").split("]")[2].replace("[",""),-1==jQuery.inArray(g,f)&&(f.push(g),p("und",g,null,jQuery(this),k),C("",g),k="persist"))});for(var e=0,l=a.logic.check.length;e<l;e++)-1==jQuery.inArray(a.logic.check[e],f)&&1>jQuery(d).find(".frm_field_"+a.logic.check[e]+"_container").length&&(p("und",a.logic.check[e],null,null,k),k="persist");0<jQuery(d).find(".star").length&&jQuery(".star").rating();0<jQuery(d).find(".frm_chzn").length&&jQuery().chosen&&jQuery(".frm_chzn").chosen({allow_single_deselect:!0});
"function"==typeof frmThemeOverride_frmAddRow&&frmThemeOverride_frmAddRow(b,a)}});return!1}function W(b){b.fadeOut("slow",function(){b.remove()})}function aa(){var b=jQuery(this).data("frmtoggle");jQuery(b).is(":visible")?jQuery(b).slideUp("fast"):jQuery(b).slideDown("fast");return!1}function J(b){return!jQuery.isArray(b)&&0<=b-parseFloat(b)+1}function O(b,a){var c=[];if("undefined"==typeof document.querySelector)jQuery("#"+b+' input[type=checkbox]:checked, input[type=hidden][name^="'+a+'"]').each(function(){c.push(this.value)});
...
jQuery(".frm-show-form input[onblur], .frm-show-form textarea[onblur]").each(function(){""===jQuery(this).val()&&jQuery(this).blur()});jQuery(document).on("focus",".frm_toggle_default",R);jQuery(document).on("blur",".frm_toggle_default",S);jQuery(".frm_toggle_default").blur();jQuery(document.getElementById("frm_resend_email")).click(T);jQuery(document).on("change",".frm_multiple_file",U);jQuery(document).on("click",".frm_clear_file_link",X);jQuery(document).on("click",".frm_remove_link",V);jQuery(document).on("focusin",
...
"visible")}}}var frmFrontForm=frmFrontFormJS();jQuery(document).ready(function(h){frmFrontForm.init()});
