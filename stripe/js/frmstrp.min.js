...
keys.length&&errors[keys[0]]==="")keys.pop();return 0===keys.length}function addName($form){let i,firstField,lastField,firstFieldContainer,lastFieldContainer,firstNameID="",lastNameID="",subFieldEl;const cardObject={};const settings=frm_stripe_vars.settings;const getNameFieldValue=function(field,subFieldName){if("object"!==typeof field)field=document.getElementById("frm_field_"+field+"_container");if(!field||"object"!==typeof field||"function"!==typeof field.querySelector)return"";subFieldEl=field.querySelector(".frm_combo_inputs_container .frm_form_subfield-"+
subFieldName+" input");if(!subFieldEl)return"";return subFieldEl.value};for(i=0;i<settings.length;i++){firstNameID=settings[i].first_name;lastNameID=settings[i].last_name}function getNameFieldItem(fieldID,type,$form=null){const queryForNameFieldIsFound="object"===typeof window.frmProForm&&"function"===typeof window.frmProForm.queryForNameField;if(type==="container")return queryForNameFieldIsFound?window.frmProForm.queryForNameField(fieldID,"container"):document.querySelector("#frm_field_"+fieldID+
"_container, .frm_field_"+fieldID+"_container");return queryForNameFieldIsFound?window.frmProForm.queryForNameField(fieldID,"field",$form[0]):$form[0].querySelector("#frm_field_"+fieldID+'_container input, input[name="item_meta['+fieldID+']"], .frm_field_'+fieldID+"_container input")}if(firstNameID!==""){firstFieldContainer=getNameFieldItem(firstNameID,"container");if(firstFieldContainer&&firstFieldContainer.querySelector(".frm_combo_inputs_container"))cardObject.name=getNameFieldValue(firstFieldContainer,
"first");else{firstField=getNameFieldItem(firstNameID,"field",$form);if(firstField&&firstField.value)cardObject.name=firstField.value}}if(lastNameID!==""){lastFieldContainer=getNameFieldItem(lastNameID,"container");if(lastFieldContainer&&lastFieldContainer.querySelector(".frm_combo_inputs_container"))cardObject.name=cardObject.name+" "+getNameFieldValue(lastFieldContainer,"last");else{lastField=getNameFieldItem(lastNameID,"field",$form);if(lastField&&lastField.value)cardObject.name=cardObject.name+
" "+lastField.value}}return cardObject}function submitForm(){if(running>0)return;frmFrontForm.submitFormManual(event,thisForm)}function enableSubmit(){if(running>0)return;thisForm.classList.add("frm_loading_form");frmFrontForm.removeSubmitLoading(jQuery(thisForm),"enable",0);triggerCustomEvent(document,"frmStripeLiteEnableSubmit",{form:thisForm})}function getPriceFields(){const priceFields=[];function checkStripeSettingForPriceFields(setting){if(-1!==setting.fields)each(setting.fields,addFieldDataToPriceFieldsArray)}
function addFieldDataToPriceFieldsArray(field){if(isNaN(field))priceFields.push("field_"+field);else priceFields.push(field)}each(getStripeSettings(),checkStripeSettingForPriceFields);return priceFields}function getStripeSettings(){const stripeSettings=[];each(frm_stripe_vars.settings,function(setting){if(-1!==setting.gateways.indexOf("stripe"))stripeSettings.push(setting)});return stripeSettings}function priceChanged(_,field,fieldId){let i;let data;const price=getPriceFields();let run=price.includes(fieldId)||
price.includes(field.id);if(!run)for(i=0;i<price.length;i++)if(field.id.indexOf(price[i])===0)run=true;if(run){data={action:"frm_strp_amount",form:JSON.stringify(jQuery(field).closest("form").serializeArray()),nonce:frm_stripe_vars.nonce};postAjax(data,function(){})}}function postAjax(data,success){const xmlHttp=new XMLHttpRequest;const params=typeof data=="string"?data:Object.keys(data).map(function(k){return encodeURIComponent(k)+"="+encodeURIComponent(data[k])}).join("&");xmlHttp.open("post",frm_stripe_vars.ajax,
...
function(fieldId){const fieldContainer=document.getElementById("frm_field_"+fieldId+"_container");if(!fieldContainer)return;const input=fieldContainer.querySelector("input[name^=item_meta]");if(input&&""!==input.value)priceChanged(null,input,fieldId)})}jQuery(document).ready(function(){const stripeParams={locale:frm_stripe_vars.locale,stripeAccount:frm_stripe_vars.account_id};frmstripe=Stripe(frm_stripe_vars.publishable_key,stripeParams);loadElements();jQuery(document).on("frmPageChanged",loadElements);
jQuery(document).off("submit.formidable",".frm-show-form");jQuery(document).on("submit.frmstrp",".frm-show-form",validateForm);jQuery(document).on("frmFieldChanged",priceChanged);checkPriceFieldsOnLoad()});window.frmStripeLiteForm={readyToSubmitStripeLink,processForm(_,e,form){event=e;thisForm=form;processForm()}}})();
