...
price.includes(fieldId)||price.includes(field.id);if(!run)for(let i=0;i<price.length;i++)if(field.id.indexOf(price[i])===0)run=true;if(run){const data={action:"frm_strp_amount",form:JSON.stringify(jQuery(field).closest("form").serializeArray()),nonce:frm_stripe_vars.nonce};postAjax(data,function(){})}}function postAjax(data,success){const xmlHttp=new XMLHttpRequest;const params=typeof data==="string"?data:Object.keys(data).map(function(k){return`${encodeURIComponent(k)}=${encodeURIComponent(data[k])}`}).join("&");
