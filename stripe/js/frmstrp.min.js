...
cardErrors;running--;enableSubmit();fieldset=jQuery(object).find(".frm_form_field");fieldset.removeClass("frm_doing_ajax");object.classList.remove("frm_loading_form");if("card_error"===error.type||"invalid_request_error"===error.type){cardErrors=object.querySelector(".frm-card-errors");if(cardErrors)cardErrors.textContent=error.message}}}function isRecurring(){var isRecurring=false;each(getStripeSettings(),function(setting){if("recurring"===setting.one){isRecurring=true;return false}});return isRecurring}
function validateFormSubmit($form){var errors,keys;errors=frmFrontForm.validateFormSubmit($form);keys=Object.keys(errors);if(1===keys.length&&errors[keys[0]]==="")keys.pop();return 0===keys.length}function addName($form){var i,firstField,lastField,firstFieldContainer,lastFieldContainer,cardObject={},settings=frm_stripe_vars.settings,firstNameID="",lastNameID="",getNameFieldValue,subFieldEl;getNameFieldValue=function(field,subFieldName){if("object"!==typeof field)field=document.getElementById("frm_field_"+
field+"_container");if(!field||"object"!==typeof field||"function"!==typeof field.querySelector)return"";subFieldEl=field.querySelector(".frm_combo_inputs_container .frm_form_subfield-"+subFieldName+" input");if(!subFieldEl)return"";return subFieldEl.value};for(i=0;i<settings.length;i++){firstNameID=settings[i].first_name;lastNameID=settings[i].last_name}function getNameFieldItem(fieldID,type,$form){$form=$form===undefined?null:$form;var queryForNameFieldIsFound="object"===typeof window.frmProForm&&
"function"===typeof window.frmProForm.queryForNameField;if(type==="container")return queryForNameFieldIsFound?window.frmProForm.queryForNameField(fieldID,"container"):document.querySelector("#frm_field_"+fieldID+"_container, .frm_field_"+fieldID+"_container");return queryForNameFieldIsFound?window.frmProForm.queryForNameField(fieldID,"field",$form[0]):$form[0].querySelector("#frm_field_"+fieldID+'_container input, input[name="item_meta['+fieldID+']"], .frm_field_'+fieldID+"_container input")}if(firstNameID!==
""){firstFieldContainer=getNameFieldItem(firstNameID,"container");if(firstFieldContainer&&firstFieldContainer.querySelector(".frm_combo_inputs_container"))cardObject.name=getNameFieldValue(firstFieldContainer,"first");else{firstField=getNameFieldItem(firstNameID,"field",$form);if(firstField&&firstField.value)cardObject.name=firstField.value}}if(lastNameID!==""){lastFieldContainer=getNameFieldItem(lastNameID,"container");if(lastFieldContainer&&lastFieldContainer.querySelector(".frm_combo_inputs_container"))cardObject.name=
